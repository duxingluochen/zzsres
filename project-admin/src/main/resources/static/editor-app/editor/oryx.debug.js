...
ORYX.CONFIG.TYPE_DEPT_MULTIINSTANCE =	"dept-multiinstance";
...
ORYX.Core.StencilSet.StencilSet = Clazz.extend({
    ...
    construct: function(source, modelMetaData, editorId){
		...
		if(ORYX.CONFIG.BACKEND_SWITCH) {
			...
			var url111=location.search;
			var Request = new Object();
			if(url111.indexOf("?")!=-1)
			{
			　　var str = url111.substr(1)　//去掉?号
			　　strs = str.split("&");
			...
			new Ajax.Request(ACTIVITI.CONFIG.contextRoot + '/editor/stencilset?code='+Request["codeOne"]+'&version=' + Date.now(), {
	            asynchronous: false,
	            method: 'get',
					...
					if (property != undefined) {
						...
						if(property.type() == ORYX.CONFIG.TYPE_CHOICE) {
							...
							property.refToView().each((function(ref) {
								...
								} else if(property.type() === ORYX.CONFIG.TYPE_DEPT_MULTIINSTANCE){
										if (ref === "multiinstance") {
										var svgElemParallel = this.node.ownerDocument.getElementById(this.id + 'parallel');
										if(svgElemParallel) 
										{
